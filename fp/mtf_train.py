...
def main() -> None:
    ...
    META.write_text(json.dumps({
        ...
        "note": "Trained on 2026-05-31..08-14. REFUTED as a gate: refit on "
                "05-31..07-20 and scored on 07-20..08-14, the model's rank "
                "correlation with the outcome was -0.0096, the band that "
                "read +0.406 of a win in sample read +0.102 at t=0.42, and "
                "trading the shipped gate lost 0.678%/trade -- worse than a "
                "rotation of its own predictions (p=0.995). mtf_bands.json "
                "is now measured OUT OF SAMPLE and no band qualifies. The "
                "prediction is kept only to ORDER candidates while the bot "
                "probes each shape and side forward on its own live record. "
                "Run python -m fp.verdict.",
    }, indent=1))
